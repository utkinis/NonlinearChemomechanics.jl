clear;figure(2);clf;colormap turbo
set(gcf,'Color','white')
% simdir = '../results/out1';
...
% simdir = '../results/out_inclusion_no_shear_no_coupling_comp1D';
simdir = '../results/out_inclusion_no_shear_coupling_comp1D';
load([simdir '/params.mat'])
[rc2,pc2] = ndgrid(rc,pc);
xc        = rc2.*cos(pc2);
yc        = rc2.*sin(pc2);
iim       = 0;
for it = [1 100:100:1000]
    load(sprintf('%s/step_%d.mat',simdir,it))
    Vx = Vr(2:end,:).*cos(pc2) - Vp(:,2:end).*sin(pc2);
    Vy = Vr(2:end,:).*sin(pc2) + Vp(:,2:end).*cos(pc2);
    tiledlayout(2,2,'TileSpacing','compact','Padding','compact')
    nexttile;pcolor(xc,yc,sqrt(Vx.^2 + Vy.^2));shading flat;axis image;colorbar;axis off;title('\rm|v|')
%     caxis([1 100])
%     xlim([0 inf]); ylim([0 inf])
%     set(gca,'ColorScale','log','FontSize',14)
    nexttile;pcolor(xc,yc,rho(2:end-1,2:end-1));shading flat;axis image;colorbar;axis off;title('\rm\rho')
%     caxis([0.99 1.1])
    set(gca,'FontSize',14)
%     xlim([0 inf]); ylim([0 inf])
